...
class SMTModelForCausalLM(PreTrainedModel):
    ...
    @torch.no_grad()
    def fast_predict(self, input, decoder_input):
        encoder_output = self.forward_encoder(input)
        output = self.forward_decoder(encoder_output=encoder_output, last_predictions=decoder_input)
        predicted_tokens = torch.argmax(output.logits, dim=-1)
        return predicted_tokens, output
